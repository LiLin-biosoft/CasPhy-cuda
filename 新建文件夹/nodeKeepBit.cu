...
#define CUDACALL(call) {                                    \
	...
	while(nTry<10){ \
		...
		if(err==cudaSuccess){ \
			...
			printf("CUDA error at %s:%d code=%d(%s) \"%s.\" Try %d times.\n",          \
				__FILE__, __LINE__, err, cudaGetErrorString(err), #call,nTry); \
			sleep(60); \
			if(nTry>=10){ \
				exit(1); \
			} \
...
int main(int argc, char *argv[]) {
	...
	srand(time(nullptr));
	int runDevice=rand()%gpuCount;
	CUDACALL(cudaSetDevice(runDevice));
...
//	printf("\n");
	...
	printf("nodeKeepBit: %.2fs elapsed for %u nodes and %u outgroups in GPU %d.\n",time_taken,nnode,nRetain,runDevice);
